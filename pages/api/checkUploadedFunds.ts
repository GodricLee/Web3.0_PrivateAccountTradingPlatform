    ...
    try {
        ...
        if (!Moralis.Core.isStarted) {
          ...
          const response = await Moralis.EvmApi.utils.runContractFunction({
            ...
            functionName: 'check_if_buyer_uploaded_funds',
        ...
        const uploaded_funds = response.result as unknown as boolean;
        const uploadedfunds = uploaded_funds ? 1 : 0;
        
        return res.status(200).json({ uploadedfunds });
    } catch (error: any) {
        return res.status(500).json({ message: 'Error checking if buyer has uploaded funds', error: error.message });
    }
}
